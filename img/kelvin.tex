...
\begin{tikzpicture}[
    ...
    \draw[densely dashed,gray] (0,0) -- (2.5,0) node[below,pos=.8,scale=0.8] {граница};
    
    \foreach \y in {0,0.25,0.5,0.75}{
        \draw[-latex] (0,\y) -- ++ (1,0);
    }
    \draw (1,0.5) node[right] {$\vec{U}$};


    % \draw (1.5,-0.5) node[scale=0.7] {$\bigotimes$} node[below=0.2em] {$\vec{\Omega}$};
  ...
  % \draw[thick] (0,0) pic[black, -latex]{carc=-20:20:1.3cm};
    ...
    \draw[fill=black] (0,0) circle(1.2pt) node[left=0.2em] {$0$};
\end{tikzpicture}
