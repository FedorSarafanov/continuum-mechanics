...
\usetikzlibrary{intersections,patterns,shadings,fadings}
...
% \draw[scale=1,,name path =B, domain=0:3,smooth,variable=\x] plot ({\x},{sin(deg(2*pi*\x/2))/5+0.5});


\draw[shading = axis, draw=none, left color=myblue, right color=myblue!10!blue,shading angle=0,] (0.2,0) rectangle +(3.5,2);
\draw[fill=blue,draw=none] (0.2,0) rectangle +(3.5,-0.4);
...
\draw[-] (-0.1,1) node [left] {$z$} -- ++(0.2,0);
\draw[-] (-0.1,1.3) node [left] {$z+x$} -- ++(0.2,0);

\draw[->] (1,1.3) -- ++ (0,0.3) node[above] {$\vec{f}_\text{арх}$};
\draw[->] (1,1.3) -- ++ (0,-0.6) node[below] {$\vec{f}_\text{тяж}$};
\draw[fill=blue!50!myblue, draw=white] (1,1.3) circle (4pt);

\begin{scope}[xshift=1cm,yshift=-0.3cm]
\draw[->] (1,1.3) -- ++ (0,0.6) node[above] {$\vec{f}_\text{арх}$};
\draw[->] (1,1.3) -- ++ (0,-0.6) node[below] {$\vec{f}_\text{тяж}$};
\draw[fill=blue!50!myblue, draw=white] (1,1.3) circle (4pt);
\end{scope}

\begin{scope}[xshift=2cm,yshift=-0.6cm]
\draw[->] (1,1.3) -- ++ (0,0.9) node[above] {$\vec{f}_\text{арх}$};
\draw[->] (1,1.3) -- ++ (0,-0.6) node[below] {$\vec{f}_\text{тяж}$};
\draw[fill=blue!50!myblue, draw=white] (1,1.3) circle (4pt);
\end{scope}
