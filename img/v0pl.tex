	...
	\begin{tikzpicture}
		\begin{axis}[
		  ...
		  unbounded coords=jump,
			...
			% extra x ticks={0.3},	% доПолнительные метКи на осях
        ...
        \addplot[domain={0:4},samples=1000,blue] {1.05} node[above,pos=0.73] {пластина};
		\addplot[domain={0:2},samples=1000,red] {-(x-1)^2+1} node[right,pos=0.8] {цилиндр};


        \draw[draw=none,fill=white] (axis cs:-0.5,-0.5) rectangle (0.5,-2.5);
        \draw[<-] (axis cs:0,-1.5) node[above] {$h$} -- (0,-3); 
        \draw[->] (axis cs:-0.5,-3) -- (5,-3) node[right] {$x$};
        \addplot[domain={0:4},samples=1000,blue] {sqrt(x)-3};
        \addplot[domain={0:1.9},samples=1000,red] {(sqrt(x)*sqrt(x/(-(x-1)^2+1)))/2-3};

		% \draw[blue,<-, >=latex] (0.7,{10*sqrt(1-0.7)}) -- (0,{10*sqrt(1-0.7)});
		% \draw[blue,<-, >=latex] (0.5,{10*sqrt(1-0.5)}) -- (0,{10*sqrt(1-0.5)});
		% \draw[blue,<-, >=latex] (0.85,{10*sqrt(1-0.85)}) -- (0,{10*sqrt(1-0.85)});
		% \draw[blue,<-, >=latex] (0.95,{10*sqrt(1-0.95)}) -- (0,{10*sqrt(1-0.95)});
		% \draw[blue,<-, >=latex] (0.24,{10*sqrt(1-0.24)}) -- (0,{10*sqrt(1-0.24)});
		% \draw[dashed] (1,2) -- (1,0);
		% \draw[magenta, fill=white,thick] (1,2) circle (1.2pt);
